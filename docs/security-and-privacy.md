...
- Fixture transcripts committed to `fixtures/` must themselves be redacted before commit (they're real sessions). A pre-commit check runs the secret scan over `fixtures/`.
